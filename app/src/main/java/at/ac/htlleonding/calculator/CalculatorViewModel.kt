...
class CalculatorViewModel : ViewModel() {
    ...
    private fun performCalculation() {
        ...
        val result = when (state.operation) {
            ...
            is CalculatorOperation.Divide -> if (number2 != 0.0) number1 / number2 else Double.POSITIVE_INFINITY
            null -> return
        }

        // Format the result to handle scientific notation and trailing zeros
        val formattedResult = when {
            result.isInfinite() -> "Error"
            result.isNaN() -> "Error"
        ...
        state = state.copy(
            number1 = formattedResult.take(MAX_NUM_LENGTH),
            number2 = "",
            operation = null
        )
    }
    ...
    private fun enterDecimal() {
        if (state.operation == null && !state.number1.contains(".")) {
            ...
            state = state.copy(
                number2 = if (state.number2.isBlank()) "0." else state.number2 + "."
            )
        }
    }
